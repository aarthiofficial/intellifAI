...
class InferenceConfig(PrintableConfig):
    ...
    checkpoint_M: str = make_abs_path('../../pretrained_weights/liveportrait/base_models/motion_extractor.pth')  # path to checkpoint of M
    ...
    version_animals: str = "_v1.1"  # new (v1.1) version
    ...
    checkpoint_M_animal: str = make_abs_path(f'../../pretrained_weights/liveportrait_animals/base_models{version_animals}/motion_extractor.pth')  # path to checkpoint of M
    ...
    driving_smooth_observation_variance: float = 3e-7  # smooth strength scalar for the animated video when the input is a source video
    source_max_dim: int = 1280  # the max dim of height and width of source image or video
    source_division: int = 2  # make sure height and width can be divided by this number
    animation_region: Literal["exp", "pose", "lip", "eyes", "all"] = "all"  # region where animation is performed

    # NOT EXPORTED PARAMS
    lip_normalize_threshold: float = 0.03  # threshold for flag_normalize_lip
    source_video_eye_retargeting_threshold: float = 0.18  # threshold for eyes retargeting if input is source video
