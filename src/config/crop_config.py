...
class CropConfig(PrintableConfig):
    ...
    vx_ratio: float = 0  # vx ratio (horizontal shift)
    vy_ratio: float = -0.125  # vy ratio (vertical shift: + up, - down)
    max_face_num: int = 0  # max face number, 0 means no limit
    flag_do_rot: bool = True  # whether to conduct rotation when cropping
    animal_face_type: str = "animal_face_9"  # animal_face_68 -> 68 landmarks, animal_face_9 -> 9 landmarks

    ########## driving video auto cropping option ##########
    scale_crop_driving_video: float = 2.2  # scale factor for cropping driving video
    vx_ratio_crop_driving_video: float = 0.0  # horizontal offset adjustment
    vy_ratio_crop_driving_video: float = -0.1  # vertical offset adjustment
